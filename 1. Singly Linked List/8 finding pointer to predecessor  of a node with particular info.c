...
void traverselist(int x)
{
    ...
    else
    {
        ...
        {
            if(temp->next->data==x)
///predecessor of temp->next->data is temp->next
                break;
            position++;
        ...
        if(temp==NULL)
            printf("Predecessor of %d is not found in the list.\n", x);
        else
            printf("Predecessor of %d is %d at postion %d.\n",x,temp->data, position);
    }
}
///same as searching
